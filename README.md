...
- **[my_utils](./my_utils/)**: This folder contains helper tools I've created for my own use. They might not be interesting to you, but you're welcome to explore them.
...
This README will give you a brief overview of the tasks of CodeAcceleratorKit and how to set them up in a new workspace.

**Note:** It is recommended to use Visual Studio Code (VSCode) as the integrated development environment (IDE) for working with CodeAcceleratorKit. VSCode provides excellent support for Python development and can be easily configured to run the tasks included in this kit.
...
First to be able to run the tasks, the CodeAcceleratorKit can be cloned with
...
Next, you will need to register the runner using our provided script `register_runner.py`, and you will need to set the required runner paths there. This script will create the necessary directories and initialize their attributes. You can find the script [here](./tasks/controllers/scripts/register_runner.py). In your storage dir you will see the following structure:


```
tasks_storage/
    ├── profile/          # Contains configuration files for the runner, such as metadata and settings.
    ├── customizations/   # Stores custom configurations and extensions for the tasks.
    ├── data/             # Serves as a storage area for task-related data and intermediate files.
    ├── outputs/          # Contains the results and outputs generated by the tasks.
    └── CHEATSHEET.md     # Provides a quick reference guide for using the tasks effectively.
```

After registering the runner, you can start using the tasks by running the corresponding task scripts with the required arguments. To quickly start a task, VSCode users can use the `tasks.json` file to set up tasks in their workspace. This file allows you to define custom tasks that can be executed directly from the VSCode interface. You can find an example of a `tasks.json` file for [windows](./tasks/management/support_files/windows/tasks.json) and for [linux](./tasks/management/support_files/linux/tasks.json). For more information about setting up tasks in VSCode, visit the [official VSCode documentation](https://code.visualstudio.com/docs/editor/tasks).
Please note that the Kit carefully organizes both the Python environments of the CodeAcceleratorKit and the workspace runner environments to avoid conflicts.
...
| `meta_macros_with_args`  | Meta macros with arguments                                                                        | `#<file_name_without_ext>_meta+`                         | `<arg_1, arg_2, ...>`                                                                                  |
